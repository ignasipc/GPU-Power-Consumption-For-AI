...
def main():
    ...
    # Convertir porcentaje de uso de GPU a número
    df_sw["uso_gpu"] = df_sw[" utilization.gpu [%]"].str.replace("%", "").astype(float)

    # Crear eje de tiempo en segundos: 1, 2, 3...
    df_hw["t_seconds"] = range(1, len(df_hw) + 1)
    df_sw["t_seconds"] = range(1, len(df_sw) + 1)

    # Imprimir la duración de ambas pruebas
    print(f"\nDuración total de la prueba Hardware: {len(df_hw)}s - Formato (HH:MM:SS): {len(df_hw)// 3600:02d}:{(len(df_hw) % 3600) // 60:02d}:{len(df_hw)% 60:02d}")
    print(f"Duración total de la prueba Software: {len(df_sw)}s - Formato (HH:MM:SS): {len(df_sw)// 3600:02d}:{(len(df_sw) % 3600) // 60:02d}:{len(df_sw)% 60:02d}")
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(df_sw["t_seconds"], df_sw["uso_gpu"], linewidth=1.5, color="blue")
    plt.xlabel("Tiempo (s)")
